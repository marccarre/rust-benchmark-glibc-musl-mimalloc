...
- [x] 10-01-PLAN.md — `axes::column_header_with_arrow` helper + `markdown::emit_per_scenario_tables` arrow-decorated headers + verbatim legend row + 4 tests (DIR-01, DIR-02, DIR-05)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 10-02-PLAN.md — `HtmlContext` + `build_context` arrow-decorated chart-axis labels + JSON header array + `index.html.tmpl` placeholder substitutions (lines 576/605/677/851) + JS aria-wrap pass + 1 a11y regression test (DIR-03, DIR-04)
...
- [x] 11-01-PLAN.md — Doc-only release gate: cargo test verification (TEST-01) + CLAUDE.md §Conventions standalone-PR bullet (TEST-02) + 11-01-SUMMARY.md byte-count capture + REQUIREMENTS.md/STATE.md updates
