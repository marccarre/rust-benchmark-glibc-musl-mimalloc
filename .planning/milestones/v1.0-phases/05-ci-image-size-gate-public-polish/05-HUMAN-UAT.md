...
- truth: "User runs `just bench-all-smoke` on the local machine (Apple Silicon) and gets 18 successful cells producing results/{alloc}-{env}.json files (REPR-01)."
  status: resolved
  resolved_in: "Phase 05.1 plan 05.1-01 (commits 178082a, fc23505, b4d2d5f, a1659be) — RUSTFLAGS_OVERRIDE build-arg added to all 6 Dockerfiles; justfile auto-detects Apple Silicon and forwards BENCH_TARGET_CPU=x86-64-v2; bench-all-smoke-apple-silicon convenience recipe added; README §Troubleshooting documents the Rosetta+v3 caveat."
...
- truth: "GHA aggregate job downloads per-cell artifacts and `just ci-aggregate` produces report/index.html + report/REPORT.md; the bench-report-{run_id} artifact uploads successfully (ORCH-04)."
  status: resolved
  resolved_in: "Phase 05.1 plan 05.1-02 (commits 6719851, 126e3f0) — bench.yml Reorganize artifacts step now sources from `artifacts/results/*.json` and `artifacts/meta/*.json` (the actual subdir paths preserved by upload-artifact@v4); `2>/dev/null || true` silencers dropped so genuine path drift surfaces as a hard fail; `ls -la artifacts/` debug print added for future-proofing."
