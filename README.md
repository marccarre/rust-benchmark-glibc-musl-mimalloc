...
- **Apple Silicon (M1/M2/M3/M4):** the build script already passes `--platform linux/amd64` to every `docker build` and `docker run` invocation (Phase 3 D-15). No user action required — the AMD64 emulation layer in Docker Desktop / Colima handles it. Expect ~2-4× slower wall-clock than a native AMD64 host; relative ordering between allocators stays valid.
