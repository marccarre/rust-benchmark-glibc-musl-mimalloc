...
# distroless-static.Dockerfile — musl-static runtime on distroless nonroot.
...
# REQUIRED: distroless-static has no libc; binary must be fully static.
...
# ─── Stage 4: runtime — distroless static (UID 65532 nonroot) ─────────
# RESEARCH §Pitfall 1: if --build-arg ALLOC=jemalloc or =mimalloc fails to
# link due to musl-static + +crt-static incompatibility, Plan 04's smoke
# pass drops the cell from the matrix per CONTEXT D-01 escape hatch and
# documents the reason in 03-04-SUMMARY.md. The Dockerfile itself remains
# valid — the failure is at link time, not Dockerfile parse time.
# RESEARCH §Pitfall 4: binary lives at /alloc-bench-cli (FS root); /usr/local/bin
# is not guaranteed in PATH for nonroot users on distroless.
FROM gcr.io/distroless/static-debian12:nonroot AS runtime
ARG OCI_VERSION
ARG OCI_REVISION
ARG OCI_CREATED
LABEL org.opencontainers.image.title="alloc-bench" \
      org.opencontainers.image.description="Memory allocator benchmark — distroless-static (musl static nonroot; mallocng/jemalloc/mimalloc)" \
...
ENV DOCKER_IMAGE=gcr.io/distroless/static-debian12:nonroot
